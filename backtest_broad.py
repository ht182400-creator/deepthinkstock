# backtest_broad.py —— 扩展宇宙池(沪深+科创+创业)日线突破，对比 2只 vs 4只
# 回答：把宇宙池扩宽到沪深+科创+创业后，"放宽到4只"能否比"2只"更有效砍回撤/提夏普？
import json, datetime, statistics, urllib.request, time

SRC='daily_broad.json'
INIT=20000; LOT=100
BUY_COST=0.0008; SELL_COST=0.0018
BRK=20; VOLW=20; TREND_MA=60; VOL_MULT=1.5
START_DATE=''   # 若设 '2019-07-22' 则只在该日之后交易（隔离区间效应）；''=全区间
...
def fetch_index(symbol='sh000852'):
...
print('宇宙池:',meta.get('universe'),'合格',len(daily))
...
print('\n=== 合格股价格分布(最新收盘) ===')
for t in [20,30,40,50,60,80,100,150]:
    print(f'  ≤{t}元: {sum(1 for v in traded_latest.values() if v<=t)} 只')
cheap50=sum(1 for v in traded_latest.values() if v<=50)
print(f'  >>> ≤50元(可放进4只×5000元仓位): {cheap50} 只 / 共 {len(traded_latest)} 只')

idx=fetch_index('sh000852')   # 中证1000 作宽基 regime proxy
...
configs=[
 ('BROAD-2 宽池2只',   dict(reg_win=200,exit_ma=7, maxpos=2,use_regime=True)),
 ('BROAD-4 宽池4只',   dict(reg_win=200,exit_ma=7, maxpos=4,use_regime=True)),
 ('BROAD-2 无regime',  dict(reg_win=200,exit_ma=7, maxpos=2,use_regime=False)),
]
results={}
...
colors={'BROAD-2 宽池2只':'#2980b9','BROAD-4 宽池4只':'#e67e22','BROAD-2 无regime':'#c0392b'}
def svg_multi(curves):
    W,H=880,380
    mx=max(max(v) for _,v in curves); mn=min(min(v) for _,v in curves); rng=(mx-mn) or 1
    def pts(vals,color):
        n=len(vals); pl=[]
...
open('report_broad.html','w').write(f'''<html><head><meta charset="utf-8"><title>宽宇宙池 2只 vs 4只 对比</title>
...
<body><h2>扩展宇宙池(沪深+科创+创业) 日线突破：2只 vs 4只（2万初始, 100股手数, 盈利复投）</h2>
<p>regime=中证1000(sh000852) 200日线；出场=突破后收盘破7日线。北交因数据源不可达已排除。</p>
{svg_multi(curves)}
<table>{hdr}{cmp_rows}</table>
<p style="color:#888">对比基准 A(STAR-only 2只, 科创50 13月线 regime): final≈4.67万, CAGR≈12.8%, MDD≈-58.5%, Sharpe≈0.382。
宽池回测期更长(含2009-2019样本外年份)，与A非同区间，仅作策略性质参考。含生存偏差。仅供参考，不构成投资建议。</p></body></html>''')
print('\nSAVED report_broad.html')
