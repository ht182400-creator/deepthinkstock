...
字段语义 (已用茅台 600519 已知分红交叉验证):
  f_cash         = 每10股现金红利(元)   [已验证: 2024-06-19=308.76, 2023-06-30=259.11, 2022-06-30=216.75, 2021-06-25=192.93]
  f_bonus        = 每10股送股(股)       [已验证: 2002-07-25=1.0 即10送1]
  f_rights_price = 配股价(元)           [配股/增发记录适用]
  f_rights       = 每10股配股(股)       [配股/增发记录; 此类记录数值较大, 单位/语义需对照已知配股再确认]
  category       = 类别字节, 含义与在线xdxr不完全一致, 不要单独用它驱动复权逻辑, 以 float 字段为准
说明: 同一除权日可能在 gbbq 中出现多条记录(如 cat1 分红 + cat2 配股), 计算复权因子时应按 除权日 归并。
"""
